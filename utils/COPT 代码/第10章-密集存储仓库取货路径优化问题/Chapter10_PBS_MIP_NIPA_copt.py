...
name: 密集存储仓库取货路径优化问题（NIPA模型）- COPT Python接口代码实现
author: 杉数科技
date:2022-10-11
institute:杉数科技
"""
...
def build_and_solve_PBS_NIPA_model(row_num, col_num, Adj_dict, Max_Step, item_set,
    ...
    # 定义NIPA模型
    model = Envr().createModel('NIPA Model')
    '''
    定义决策变量
    -------------------------
    ...
    # with open(solution_file, 'w') as f:   # 模式'x'，每次都重新从头写入
        if (model.Status == COPT.OPTIMAL):  # status code == 2, 表示求解到了最优解
            ...
            for k in range(Optimal_step_num + 1):   #Max_Step
                PBS_warehouse_k = np.zeros([row_num, col_num], dtype=int)
                for p in pos_set:
                    temp = 0
                    for i in item_set:
