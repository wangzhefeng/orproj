...
crew_list, flight_list, crew_base = read_data(
    file_name_1, file_name_2, crew_select_interval,
    flight_select_interval
)  # 读取数据
...
problem_ID = 1
Adj_matrix, Adj_matrix_hop, Adj_matrix_hop_bi, Adj_matrix_hop_nbi = generate_adj_matrix(
    flight_list, crew_base, max_arc_num, problem_ID
)
    ...
    for j in flightid:
        if j != crew_list[empid.index(r)].Base:
            if (crew_list[empid.index(r)].Base, j) in Adj_matrix:
                lhs.addTerms(x[crew_list[empid.index(r)].Base, j, r], 1)
    ...
    for i in flightid:
        if i != crew_list[empid.index(r)].Base:
            if (i, crew_list[empid.index(r)].Base) in Adj_matrix:
                lhs2.addTerms(x[i, crew_list[empid.index(r)].Base, r], 1)
    ...
    for i in flightid:
        for k in range(1, 4):
            if i != crew_list[empid.index(empid[rid])].Base:
                expr1.addTerms(z[i, empid[rid], k], 1)

                expr2.addTerms(z[i, empid[rid - 1], k], 1)
