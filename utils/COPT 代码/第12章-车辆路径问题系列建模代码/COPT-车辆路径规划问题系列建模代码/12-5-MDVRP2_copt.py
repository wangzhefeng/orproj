...
name: 多车场车辆路径规划问题（MDVRP2）- COPT - Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技 
"""
...
def MDVRP_2(N,K,Demand,Distance,Deta):
    try:
        env = Envr()
        m = env.createModel("MDVRP2")
        # 创建变量
        x = {}
        q = {}
        # 将第一个点和第二个点作为都作为车场。节点数共有N+D个，顾客数等于N-D个
        S = N - D
        N = N + D
        for k in range(K):
            for i in range(N):
                # 创建车辆到达点i剩余容量变量q_i,是连续类型变量，算例设置的车辆容量最大为200，因只使用了其中14个顾客点，将容量限制为80。
                q[i, k] = m.addVar(lb=0.0, ub=80, obj=0.0, vtype=COPT.CONTINUOUS, name="q_%d_%d" % (i, k))
            for i in range(N - D):
                for j in range(D, N):
                    # 默认不存在点i到点i的弧。
                    if i != j:
                        ...
                        if i in range(D):
                            if j in range(D, N - D):
                                x[i, j, k] = m.addVar(obj=Distance[i][j], vtype=COPT.BINARY,
                            ...
                            if j in range(D, N - D):
                                ...
                                x[i, j, k] = m.addVar(obj=Distance[i][j - S - D], vtype=COPT.BINARY,
                                                      name="x_%d_%d_%d" % (i, j, k))
        # 设置目标函数最小化
        ...
        for k in range(K):
            for i in range(D, N - D):
                ...
                for j in range(D, N):
                    if i != j:
                        rh.addTerms(x[i, j, k], 1)
                for j in range(N - D):
                    if i != j:
                        lh.addTerms(x[j, i, k], 1)
...
# 开始在Copt中建模
MDVRP_2(N,K,Demand,Distance,Deta)
