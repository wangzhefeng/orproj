...
name: 带软时间窗的车辆路径规划问题（VRPSTW1）- COPT - Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技 
"""
...
def VRPSTW_1(N,A,B,Demand,Distance,ST,MAX_B,Cost_B):
    try:
        env = Envr()
        m = env.createModel("VRPSTW1")
        ...
        for i in range(N):
            ...
            if i in range(1, N - 1):
                t[i] = m.addVar(lb=A[i], ub=B[i] + MAX_B, obj=0.0, vtype=COPT.CONTINUOUS, name="t_%d" % i)
            else:
...
# 开始在Copt中建模
VRPSTW_1(N,A,B,Demand,Distance,ST,MAX_B,Cost_B)
