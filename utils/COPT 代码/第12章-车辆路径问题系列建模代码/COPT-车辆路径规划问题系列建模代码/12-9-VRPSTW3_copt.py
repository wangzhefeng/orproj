...
name: 带软时间窗的车辆路径规划问题（VRPSTW3）- COPT - Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技 
"""
...
def VRPSTW_3(N,A,B,Demand,Distance,ST,MAX_B,Cost_B,MAX_A,Cost_A):
    try:
        env = Envr()
        m = env.createModel("VRPSTW3")
                ...
                if i != j:
                    if i == 0:
                        if j == N - 1:
                            ...
                            m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 400,
                                        "Time1_%d_%d" % (i, j))
                            # m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] >= (x[i, j]-1) * 400, "Time2_%d_%d" % (i, j))
                    else:
                        if j == N - 1:
                            m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] <= (1 - x[i, j]) * 400,
                                        "Time1_%d_%d" % (i, j))
                            # m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] >= (x[i, j]-1) * 400, "Time2_%d_%d" % (i, j))
                        else:
                            m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 400,
                                        "Time1_%d_%d" % (i, j))
                            m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] >= (x[i, j] - 1) * 400,
                                        "Time2_%d_%d" % (i, j))
        # 软时间窗上界确定（晚到）
        for i in range(1, N - 1):
...
# 开始在Copt中建模
VRPSTW_3(N,A,B,Demand,Distance,ST,MAX_B,Cost_B,MAX_A,Cost_A)
