...
name: 带硬时间窗的车辆路径规划问题（VRPHTW）- COPT - Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技 
"""
...
def VRPHTW(N,A,B,Demand,Distance,ST):
    try:
        env = Envr()
        m = env.createModel("VRPHTW")
                ...
                if i != j:
                    if i == 0:
                        if j == N - 1:
                            ...
                            m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 300,
                                        "Time_%d_%d" % (i, j))
                    else:
                        if j == N - 1:
                            m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] <= (1 - x[i, j]) * 300,
                                        "Time_%d_%d" % (i, j))
                        else:
                            m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 300,
                                        "Time_%d_%d" % (i, j))
...
# 开始在Copt中建模
VRPHTW(N,A,B,Demand,Distance,ST)
