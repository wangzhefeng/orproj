...
name: 多车场车辆路径规划问题（MDVRP1）- COPT - Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技 
"""
...
def MDVRP_1(N,K,Demand,Distance,Deta):
    try:
        env = Envr()
        m = env.createModel("MDVRP1")
        # 创建变量
        x = {}
        q = {}
        # 将第一个点和第二个点作为都作为车场。
        for i in range(N):
            ...
            q[i] = m.addVar(lb=0.0, ub=80, obj=0.0, vtype=COPT.CONTINUOUS, name="q_%d" % i)
            for j in range(N):
                # 默认不存在点i到点i的弧。
                if i != j:
                    # 以下两个是排除车场到车场的变量
...
# 开始在Copt中建模
MDVRP_1(N,K,Demand,Distance,Deta)
