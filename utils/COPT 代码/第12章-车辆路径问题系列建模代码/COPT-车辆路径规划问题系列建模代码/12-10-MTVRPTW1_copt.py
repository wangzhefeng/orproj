...
name: 带时间窗的多行程车辆路径问题（MTVRPTW1）- COPT - Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技 
"""
...
def MTVRPTW_1(N,A,B,Demand,Distance,ST,R,K,Tau,F):
    try:
        env = Envr()
        m = env.createModel("MTVRPTW1")
        ...
        # 每个顾客都必须被有且只有一辆车服务，使用可能离开点i的路径刻画
        ...
        for k in range(K):
            for i in range(R):
                for j in range(R):
                    if i != j:
                        m.addConstr(t[N - 1, i] + Tau - t[0, j] <= (1 - z[i, j, k]) * 300,
                                    "Sequence_%d_%d_%d" % (i, j, k))
        # 车程分配顺序弧数量约束
        for k in range(K):
            rh = LinExpr()
            lh = LinExpr()
            ...
            for i in range(R):
                for j in range(R):
                    if i != j:
                        rh.addTerms(z[i, j, k], 1.0)
            m.addConstr(lh - 1 <= rh, "ArcAssignment_%d" % k)
        # 车程分配流守恒约束
        for k in range(K):
            for i in range(R):
                rh1 = LinExpr()
                ...
                for j in range(R):
                    if i != j:
                        rh1.addTerms(z[j, i, k], 1.0)
                        rh2.addTerms(z[i, j, k], 1.0)
                m.addConstr(y[k, i] >= rh1, "ArcFlowRight_%d_%d" % (i, k))
                m.addConstr(y[k, i] >= rh2, "ArcFlowLeft_%d_%d" % (i, k))
        log_file_name = 'MTVRPTW1.log'
...
# 开始在Copt中建模
MTVRPTW_1(N,A,B,Demand,Distance,ST,R,K,Tau,F)
