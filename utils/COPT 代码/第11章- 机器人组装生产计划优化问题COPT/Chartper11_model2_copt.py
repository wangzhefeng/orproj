...
name: 机器人组装生产计划优化问题（问题二） - COPT Python接口代码实现
author: 杉数科技
date:2022-10-11
"""
...
def problem2_build_model_and_solve():
    ...
    for i in range(7):
        for j in range(3):
            name = f'x_{i}_{j}'
            x[i, j] = model.addVar(lb=0, ub=500000, vtype=copt.COPT.INTEGER, name=name)

    for i in range(7):
        for j in range(3):
            name = f'y_{i}_{j}'
            y[i, j] = model.addVar(lb=0, ub=500000, vtype=copt.COPT.INTEGER, name=name)

    for i in range(7):
        name = f'w_{i}'
        w[i] = model.addVar(lb=0, ub=500000, vtype=copt.COPT.INTEGER, name=name)

    for i in range(7):
        name = f'yw_{i}'
        yw[i] = model.addVar(lb=0, ub=500000, vtype=copt.COPT.INTEGER, name=name)
    ...
    for i in range(7):
        for j in range(8):
            name = f'z_{i}_{j}'
            z[i, j] = model.addVar(lb=0, ub=500000, vtype=copt.COPT.INTEGER, name=name)

    for i in range(7):
        for j in range(8):
            name = f'yz_{i}_{j}'
            yz[i, j] = model.addVar(lb=0, ub=500000, vtype=copt.COPT.INTEGER, name=name)
    ...
    for i in range(7):
        for j in range(3):
            model.addConstr(k[j] * w[i] <= y[i, j])

    # 大组件隔天投入生产
    for i in range(7):
        for j in range(3):
            for q in range(8):
                model.addConstr(x[i, j] * h[j][q] <= yz[i, q])

    # 判断是否生产大组件（线性化后）
    for i in range(7):
        for j in range(3):
            model.addConstr(x[i, j] >= 0.5 * l[i, j])
            model.addConstr(x[i, j] <= 500000 * l[i, j])

    # 判断是否生产WPCR(线性化后)
    for i in range(7):
        model.addConstr(w[i] >= 0.5 * lw[i])
        model.addConstr(w[i] <= 500000 * lw[i])

    # 判断是否生产小组件(线性化后)
    for i in range(7):
        for j in range(8):
            model.addConstr(z[i, j] >= 0.5 * lz[i, j])
            model.addConstr(z[i, j] <= 500000 * lz[i, j])

    # i+1天WPCR库存公式
    for i in range(6):
        model.addConstr(yw[i + 1] == w[i] + yw[i] - NW[i])
    model.addConstr(yw[0] == w[6] + yw[6] - NW[6])
    ...
    for j in range(3):
        model.addConstr(y[0, j] == x[6, j] + y[6, j] - w[6] * k[j])
    ...
    for q in range(8):
        model.addConstr(yz[0, q] == yz[6, q] - sum(h[j][q] * x[6, j] for j in range(3)))
