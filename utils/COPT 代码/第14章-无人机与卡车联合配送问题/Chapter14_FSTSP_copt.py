...
name: 无人机与卡车联合配送问题 - COPT Python接口代码实现
author: 杉数科技
date: 2022-10-11
institute: 杉数科技
"""
...
class Model_builder(object):
    ...
    def build_model(self, data=None, solve_model=False):
        ...
        env = Envr()
        # 创建模型对象
        self.model = env.createModel("FSTSP")
        ...
        for i in range(data.node_num):
            ...
            name2 = 'T_' + str(i)
            name3 = 'Tt_' + str(i)
            self.U[i] = self.model.addVar(lb=0, ub=data.node_num, vtype=COPT.CONTINUOUS, name=name1)
            self.T[i] = self.model.addVar(lb=0, ub=self.big_M, vtype=COPT.CONTINUOUS, name=name2)
            self.Tt[i] = self.model.addVar(lb=0, ub=self.big_M, vtype=COPT.CONTINUOUS, name=name3)
            for j in range(data.node_num):
                name4 = 'X_' + str(i) + "_" + str(j)
                name5 = 'P_' + str(i) + "_" + str(j)
                self.X[i][j] = self.model.addVar(lb=0, ub=1, vtype=COPT.BINARY, name=name4)
                self.P[i][j] = self.model.addVar(lb=0, ub=1, vtype=COPT.BINARY, name=name5)
                for k in range(data.node_num):
                    name6 = 'Y_' + str(i) + "_" + str(j) + "_" + str(k)
                    self.Y[i][j][k] = self.model.addVar(lb=0, ub=1, vtype=COPT.BINARY, name=name6)

        # 创建目标函数
        self.model.setObjective(self.T[data.node_num - 1], COPT.MINIMIZE)
        ...
        for j in range(1, data.node_num - 1):  # 这里需要注意，i的取值范围，否则可能会加入空约束
            ...
            for i in range(0, data.node_num - 1):           # i in N0
                if (i != j):
                    expr.addTerms(self.X[i][j], 1)
                    for k in range(1, data.node_num):       # k in N+
                        if (i != k and j != k):
                            expr.addTerms(self.Y[i][j][k], 1)
        ...
        for j in range(1, data.node_num):
            expr.addTerms(self.X[0][j], 1)
        self.model.addConstr(expr == 1, "c2")

        # 约束 3: 卡车到达终点一次
        expr = LinExpr(0)
        for i in range(data.node_num - 1):
            expr.addTerms(self.X[i][data.node_num - 1], 1)
        ...
        for j in range(1, data.node_num - 1):
            ...
            for i in range(0, data.node_num - 1):
                if (j != i):
                    expr1.addTerms(self.X[i][j],1)

            for k in range(1, data.node_num):
                if (j != k):
                    expr2.addTerms(self.X[j][k],1)
                ...
                for k in range(1, data.node_num):
                    if (i != j and i != k and j != k):
                        ...
                        for h in range(data.node_num - 1):
                            if (h != i):
                                expr1.addTerms(self.X[h][i],1)
                        for l in range(1, data.node_num - 1):
                            if (l != k):
                                expr2.addTerms(self.X[l][k],1)
        ...
        for i in range(1, data.node_num - 1):
            for k in range(1, data.node_num):
                if (k != i):
                    ...
                    for j in range(1, data.node_num - 1):
                        if (i != j and j != k):
                            expr.addTerms(self.Y[i][j][k], self.big_M)
                    ...
                    for l in range(1, data.node_num - 1):
                        for m in range(1, data.node_num):
                            if (k != l and k != m and l != m):
                                expr1.addTerms(self.Y[k][l][m], data.lunching_time)

                    for i in range(data.node_num - 1):
                        for j in range(1, data.node_num - 1):
                            if (i != j and i != k and j != k):
                                expr2.addTerms(self.Y[i][j][k], data.lunching_time)
        ...
        self.model.Param.TimeLimit = 1200
        if(solve_model == True):
            self.model.solve()
