...
name: 密集存储仓库取货路径优化问题（NIPF模型）- Gurobi Python接口代码实现
author: 刘兴禄
date:2020-11-25
"""
...
def build_and_solve_PBS_NIPF_model(row_num, col_num, Adj_dict, Max_Step,
    ...
    for k in range(Max_Step):
        for p in pos_set:
            x[k, p] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY, name='x_' + str(k) + '_' + str(p))
            for r in range(len(desired_item_set)):
                y[k, p, r] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY,
                                          name='y_' + str(k) + '_' + str(p) + '_' + str(r))
            for q in pos_set:
                if (p != q):
                    z[k, p, q] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY
                                              , name='z_' + str(k) + '_' + str(p) + '_' + str(q)
                                              )
                    for r in range(len(desired_item_set)):
                        w[k, p, q, r] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY,
                                                     name='w_' + str(k) + '_' + str(p) + '_' + str(q) + '_' + str(r))

    # 创建目标函数
    obj = LinExpr(0)
    for key in z.keys():
        obj.addTerms(1, z[key])
    ...
    for k in range(1, Max_Step):  # 注意从1开始
        for p in pos_set:
            lhs = LinExpr()
            lhs.addTerms(1, x[k - 1, p])
            for q in pos_set:
                if (p != q):
                    lhs.addTerms(-1, z[k, p, q])
                    lhs.addTerms(1, z[k, q, p])
    ...
    for k in range(1, Max_Step):
        for p in pos_set:
            for r in range(len(desired_items_pos)):
                lhs = LinExpr()
                lhs.addTerms(1, y[k - 1, p, r])
                for q in pos_set:
                    if (p != q):
                        lhs.addTerms(-1, w[k, p, q, r])
                        lhs.addTerms(1, w[k, q, p, r])
    ...
    model.setParam(GRB.Param.TimeLimit, 7200 * 2)
    model.optimize()

    end_time = time.time()
    # CPU_time = end_time - start_time
    CPU_time = model.Runtime
    print('求解时间 :', CPU_time)
    ...
    # with open(solution_file, 'w') as f:
        if (model.Status == 2 or model.Status == 9):  # 状态码 == 2, 意味着模型求解到了OPTIMAL
            print('\n\n\n------------------------')
            ...
            if (model.SolCount > 0):
                ...
                print('Gurobi获得的解的个数:', model.SolCount)
                print('求解时间: {} 秒'.format(CPU_time))
                f.write('\n Gurobi获得的解的个数:' + str(model.SolCount) + '\n\n')
...
# 求解模型
Max_Step = 14
CPU_time, Opt_Obj = build_and_solve_PBS_NIPF_model(
    row_num, col_num, Adj_dict, Max_Step,
    pos_set, init_occupy_state, desired_item_set,
    IO_point_set, empty_cell_pos
)
