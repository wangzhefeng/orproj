...
name: 生产计划优化问题--Gurobi Python接口代码实现
author: 刘兴禄
date: 2019-10-15
"""
...
def build_production_plan_model_and_solve(instance=None):
    ...
    for i in range(instance.period_num):
        ...
        y[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='y_'+str(i))
        I[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='I_'+str(i))
        e[i] = model.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='e_'+str(i))
        H[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='H_'+str(i))
        F[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='F_'+str(i))
        L[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='L_'+str(i))
        P[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='P_'+str(i))
        S[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name='S_'+str(i))
        O[i] = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name='O_'+str(i))
        z[i] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY, name='z_'+str(i))
    ...
    for i in range(1, instance.period_num):
        obj.addTerms(instance.price, S[i])
        obj.addTerms(-instance.raw_material_cost, x[i])
        obj.addTerms(-instance.outsource_unit_cost, y[i])
        obj.addTerms(-instance.overtime_unit_salary, O[i])
        obj.addTerms(-instance.normal_unit_salary * instance.work_day_num * instance.work_time_each_day, P[i])
        obj.addConstant(-instance.init_inventory * instance.unit_inventory_cost)
        obj.addTerms(-instance.unit_inventory_cost, I[i])
        obj.addTerms(-instance.unit_shortage_cost, L[i])
        obj.addTerms(-instance.hire_cost, H[i])
        obj.addTerms(-instance.fire_cost, F[i])
