...
try:
    ...
    for i in range(N):
        ...
        Beta[i] = m.addVar(lb=0, ub=10, obj=0.5, vtype=GRB.CONTINUOUS, name="Beta_%d" % i)
        # 同理创建早到时间变量，早到一般没有迟到成本高，而且早到比较常见，因此设置成本为0.3，时间为15
        Alpha[i] = m.addVar(lb=0, ub=15, obj=0.3, vtype=GRB.CONTINUOUS, name="Alpha_%d" % i)
        # 创建车辆到达时间变量，算例中给出了上下界，其中晚到的时间可以最大为10
        if i in range(1, N - 1):
            t[i] = m.addVar(lb=A[i]-15, ub=B[i] + 10, obj=0.0, vtype=GRB.CONTINUOUS, name="t_%d" % i)
        else:
            ...
            if i != j:
                if i == 0:
                    if j == N - 1:
                        ...
                        m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 400, "Time1_%d_%d" % (i, j))
                        # m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] >= (x[i, j]-1) * 400, "Time2_%d_%d" % (i, j))
                else:
                    if j == N - 1:
                        m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] <= (1 - x[i, j]) * 400, "Time1_%d_%d" % (i, j))
                        # m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] >= (x[i, j]-1) * 400, "Time2_%d_%d" % (i, j))
                    else:
                        m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 400, "Time1_%d_%d" % (i, j))
                        m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] >= (x[i, j]-1) * 400, "Time2_%d_%d" % (i, j))
    # 软时间窗上界确定（晚到）
