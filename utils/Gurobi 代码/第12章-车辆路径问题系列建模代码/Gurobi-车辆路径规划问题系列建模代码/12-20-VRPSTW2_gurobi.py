...
name: 带软时间窗的车辆路径规划问题（VRPSTW2）- Gurobi - Python接口代码实现
author: 张一白
date: 2022-11-11
"""
...
try:
            ...
            if i != j:
                if i == 0:
                    if j == N - 1:
                        ...
                        m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 200, "Time_%d_%d" % (i, j))
                else:
                    if j == N - 1:
                        m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] <= (1 - x[i, j]) * 200, "Time_%d_%d" % (i, j))
                    else:
                        m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 200, "Time_%d_%d" % (i, j))
    # 软时间窗上界确定（晚到）
    for i in range(1, N - 1):
    ...
    for i in range(N):
        print("Name:%d , value:%d" % (i, Beta[i].x), end="\n")
