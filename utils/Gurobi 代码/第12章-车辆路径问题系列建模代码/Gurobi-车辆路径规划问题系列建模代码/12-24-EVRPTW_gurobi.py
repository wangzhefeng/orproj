...
try:
    ...
    for i in range(N):
        ...
        q[i] = m.addVar(lb=0.0, ub=50, obj=0.0, vtype=GRB.CONTINUOUS, name="q_%d" % i)
        # 创建车辆到达点i已消耗的电量,电量为0-100
        y[i] = m.addVar(lb=0.0, ub=100, obj=0.0, vtype=GRB.CONTINUOUS, name="y_%d" % i)
        # 创建车辆到达点i的时间变量
        if i in range(0, C):
            t[i] = m.addVar(lb=A[i], ub=B[i], obj=0.0, vtype=GRB.CONTINUOUS, name="t_%d" % i)
        elif i in range(C, N - 1):
            t[i] = m.addVar(lb=A[C], ub=B[C], obj=0.0, vtype=GRB.CONTINUOUS, name="t_%d" % i)
        else:
        ...
        for j in range(1, N):
            ...
            if i != j:
                ...
                if i == 0:
                    # 以客户点为终点
                    if j < C:
                        x[i, j] = m.addVar(obj=Distance[0][j], vtype=GRB.BINARY, name="x_%d_%d" % (i, j))
                    # 以充电站为终点
                    elif j < N - 1:
                        x[i, j] = m.addVar(obj=Distance[0][C], vtype=GRB.BINARY, name="x_%d_%d" % (i, j))
                # 以客户点开始的弧
                elif i in range(1, C):
                    # 以客户点为终点
                    if j < C:
                    ...
                    # 以充电站为终点
                        ...
                        x[i, j] = m.addVar(obj=Distance[i][C], vtype=GRB.BINARY, name="x_%d_%d" % (i, j))
                    # 以场站为终点
                    elif j == N - 1:
                ...
                # 以充电站开始的弧
                    ...
                    # 以客户点为终点
                    if j in range(1, C):
                        x[i, j] = m.addVar(obj=Distance[C][j], vtype=GRB.BINARY, name="x_%d_%d" % (i, j))
                    # 以场站为终点
                    elif j == N - 1:
                        x[i, j] = m.addVar(obj=Distance[C][0], vtype=GRB.BINARY, name="x_%d_%d" % (i, j))
        ...
        for j in range(1, N):
            if i != j:
                ...
                # 如果是客户点开始的弧：
                    ...
                    if j in range(1, C):
                        m.addConstr(t[i] + ST[i] + Distance[i][j] - t[j] <= (1 - x[i, j]) * 500, "Time_%d_%d" % (i, j))
                    elif j in range(C, N - 1):
                        m.addConstr(t[i] + ST[i] + Distance[i][C] - t[j] <= (1 - x[i, j]) * 500, "Time_%d_%d" % (i, j))
                    else:
                        m.addConstr(t[i] + ST[i] + Distance[i][0] - t[j] <= (1 - x[i, j]) * 500, "Time_%d_%d" % (i, j))
                # 如果是充电站开始的弧：
                elif i in range(C, N - 1):
                    if j in range(1, C):
